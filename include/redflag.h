...
void rf_parse_flags(rf_context_t *ctx, int argc, const char **argv);
...
static const char *_rf_shift_args(int *argc, const char ***argv) {
...
void rf_parse_flags(rf_context_t *ctx, int argc, const char **argv) {
    ...
    while(argc > 0) {
        const char *arg = _rf_shift_args(&argc, &argv);
        bool found = false;
        rf_flag_t *flag;
        ...
        if(!found)
            fprintf(stderr, "Failed to find option/flag of name \"%s\"\n", arg);
    }
}
